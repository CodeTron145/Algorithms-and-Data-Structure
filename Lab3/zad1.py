...
def excel_data():
    ...
    headings = ['Nazwa', 'Dlugosc', 'Czas', 'L_Porownan', 'L_Przestawien', 'Pamiec']
    ...
    for i in arr:
        ...
        if sys.argv[2] == "insert" and sys.argv[4] == "<=":
            ...
            for j in range(int(sys.argv[7])):
                ...
                size_array.append(length)
            algorithm_name = 'Hybrid'
        elif sys.argv[2] == "radix" and sys.argv[4] == "<=":
            for j in range(int(sys.argv[7])):
                timer = timeit.Timer(lambda: radix_sort(nums)).timeit(number=1)
                memory.append(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)
                time_ex.append(timer)
                shifts.append(shift_counter)
                compares.append(comp_counter)
            ...
            for j in range(int(sys.argv[7])):
                memory.append(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)
                timer = timeit.Timer(lambda: radix_sort_reverse(nums)).timeit(number=1)
                time_ex.append(timer)
                shifts.append(shift_counter)
                compares.append(comp_counter)
                size_array.append(length)
...
def comp_n():
    ...
    elif sys.argv[2] == "radix":
        table = pd.read_excel(path + "radixSort.xls")
...
def d_shift_n():
    ...
    table_new = table[['L_Przestawien', 'Dlugosc']]
...
# Dla rysowania wykresów Pan powinien wpisać ścieżkę gdzie znajduje plik.xls
path = "/home/s246389/Lab3/"
